...
// 辅助函数：构建带参数的API URL (与 app.js 中的版本相同)
...
async function handleApiRequest(url) {
    ...
    try {
        ...
        if (url.pathname === '/api/detail') {
            ...
            let baseApiUrlForDetail;
            if (customApi) {
                baseApiUrlForDetail = customApi;
            } else {
                baseApiUrlForDetail = API_SITES[sourceCode].api;
            }

            const detailParams = { [API_CONFIG.detail.params.ids]: id };
            if (API_CONFIG.detail.ac) {
                detailParams['ac'] = API_CONFIG.detail.ac;
            }
            const detailUrl = buildApiUrl(baseApiUrlForDetail, detailParams);
