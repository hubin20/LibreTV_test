// 全局变量
let selectedAPIs = JSON.parse(localStorage.getItem('selectedAPIs') || '[]'); // 初始化为空数组，将在后面处理默认值
...
function initAPICheckboxes() {
    ...
    // 如果 selectedAPIs 为空（可能在旧版本升级或首次加载前未正确初始化），则填充默认普通资源
    if (selectedAPIs.length === 0 && !localStorage.getItem('hasInitializedDefaults')) {
        const normalApiKeys = Object.keys(API_SITES).filter(key => !API_SITES[key].adult);
        selectedAPIs = normalApiKeys;
        localStorage.setItem('selectedAPIs', JSON.stringify(selectedAPIs));
    }
...
function addAdultAPI() {
    ...
    if (!HIDE_BUILTIN_ADULT_APIS && !yellowFilterEnabled) {
        const container = document.getElementById('apiCheckboxes');
        let adultdiv = document.getElementById('adultdiv'); // 尝试获取已存在的成人区

        // 如果成人区不存在，则创建
        if (!adultdiv) {
            adultdiv = document.createElement('div');
            adultdiv.id = 'adultdiv';
            adultdiv.className = 'grid grid-cols-2 gap-2 mt-4'; // 添加一些上边距
            const adultTitle = document.createElement('div');
            adultTitle.className = 'api-group-title adult col-span-2'; // 标题占满两列
            adultTitle.innerHTML = `黄色资源采集站 <span class="adult-warning">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" class="inline-block w-4 h-4 ml-1">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
            </svg>
        </span>`;
            adultdiv.appendChild(adultTitle);
            container.appendChild(adultdiv); // 将成人区添加到容器
        } else {
            // 如果已存在，清空其内容以便重新填充，避免重复添加API
            const checkboxes = adultdiv.querySelectorAll('.flex.items-center');
...
function checkAdultAPIsSelected() {
    ...
    const yellowFilterToggle = document.getElementById('yellowFilterToggle');
    if (!yellowFilterToggle) return; // 如果开关不存在，则退出
    ...
    if (hasAdultSelected) {
        ...
        // localStorage.setItem('yellowFilterEnabled', 'false'); // 当选中成人API时，这个状态应该已经是false了
        ...
        // localStorage.setItem('yellowFilterEnabled', yellowFilterToggle.checked.toString()); // 根据开关实际状态更新存储
...
async function search() {
    ...
    try {
        ...
        const searchPromises = selectedAPIs.map(async (apiId) => {
            try {
                ...
                if (apiId.startsWith('custom_')) {
                    ...
                    const searchParams = { [API_CONFIG.search.params.query]: query };
                    if (API_CONFIG.search.ac) {
                        searchParams['ac'] = API_CONFIG.search.ac;
                    }
                    apiUrl = buildApiUrl(apiBaseUrl, searchParams);
                ...
                if (pagesToFetch > 0) {
                    ...
                    for (let page = 2; page <= pagesToFetch + 1; page++) {
                        ...
                        //    .replace('{page}', page);
                        const pageParams = {
                            [API_CONFIG.search.params.query]: query,
                            [API_CONFIG.search.params.page]: page
                        };
                        if (API_CONFIG.search.ac) {
                            pageParams['ac'] = API_CONFIG.search.ac;
                        }
                        const pageUrl = buildApiUrl(apiBaseUrl, pageParams);
        ...
        // 然后，对过滤后的、相关的结果进行排序
        allResults.sort((a, b) => {
            const titleA = (a.vod_name || '').toLowerCase();
            const titleB = (b.vod_name || '').toLowerCase();

            // 1. 完全匹配的优先级最高
            const aIsExactMatch = titleA === originalQuery;
            const bIsExactMatch = titleB === originalQuery;

            if (aIsExactMatch && !bIsExactMatch) return -1; // A完全匹配，B不是，A在前
            if (!aIsExactMatch && bIsExactMatch) return 1;  // B完全匹配，A不是，B在前
            if (aIsExactMatch && bIsExactMatch) return 0;   // 两者都完全匹配，视为同等

            // 2. 如果都不是完全匹配，则比较是否以查询词开头
            // (此时所有条目都已确认包含查询词)
            const aStartsWith = titleA.startsWith(originalQuery);
            const bStartsWith = titleB.startsWith(originalQuery);

            if (aStartsWith && !bStartsWith) return -1; // A以查询词开头，B不是，A在前
            if (!aStartsWith && bStartsWith) return 1;  // B以查询词开头，A不是，B在前

            // 3. 如果都是以查询词开头，或者都不是以查询词开头（但都包含查询词）
            // 则按标题长度排序，越短的（越接近关键词）越靠前
